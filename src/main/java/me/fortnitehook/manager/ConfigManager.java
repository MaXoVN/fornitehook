...
import me.fortnitehook.features.setting.EnumConverter;
...
public class ConfigManager implements Util {
    ...
    public String config = "fortnitehook/config/";
    ...
    public void loadConfig(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("fortnitehook").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        if (files.contains(new File("fortnitehook/" + name + "/"))) {
            this.config = "fortnitehook/" + name + "/";
        } else {
            this.config = "fortnitehook/config/";
        }
    ...
    public boolean configExists(String name) {
        final List<File> files = Arrays.stream(Objects.requireNonNull(new File("fortnitehook").listFiles())).filter(File::isDirectory).collect(Collectors.toList());
        return files.contains(new File("fortnitehook/" + name + "/"));
    }
    ...
    public void saveCurrentConfig() {
        File currentConfig = new File("fortnitehook/currentconfig.txt");
        try {
            if (currentConfig.exists()) {
                FileWriter writer = new FileWriter(currentConfig);
                String tempConfig = this.config.replaceAll("/", "");
                writer.write(tempConfig.replaceAll("fortnitehook", ""));
                writer.close();
            } else {
