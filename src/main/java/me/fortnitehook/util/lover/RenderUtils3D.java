...
public class RenderUtils3D extends Tessellator
{
    ...
    public static void drawGradientPlane(final BlockPos pos, final EnumFacing face, final Color startColor, final Color endColor, final double height) {
        ...
        final Vec3d interp = EntityUtil.interpolateEntity(RenderUtils3D.mc.player, RenderUtils3D.mc.getRenderPartialTicks());
        final AxisAlignedBB bb = iblockstate.getSelectedBoundingBox(RenderUtils3D.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z).expand(0.0, height, 0.0);
    ...
    public static void drawGradientBlockOutline(final BlockPos pos, final Color startColor, final Color endColor, final float linewidth, final double height) {
        final IBlockState iblockstate = RenderUtils3D.mc.world.getBlockState(pos);
        final Vec3d interp = EntityUtil.interpolateEntity(RenderUtils3D.mc.player, RenderUtils3D.mc.getRenderPartialTicks());
        drawGradientBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtils3D.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z).expand(0.0, height, 0.0), startColor, endColor, linewidth);
    }
    ...
    public static void drawBlockOutline(final BlockPos pos, final Color color, final float linewidth, final boolean air) {
        ...
        if ((air || iblockstate.getMaterial() != Material.AIR) && RenderUtils3D.mc.world.getWorldBorder().contains(pos)) {
            final Vec3d interp = EntityUtil.interpolateEntity(RenderUtils3D.mc.player, RenderUtils3D.mc.getRenderPartialTicks());
            drawBlockOutline(iblockstate.getSelectedBoundingBox(RenderUtils3D.mc.world, pos).grow(0.0020000000949949026).offset(-interp.x, -interp.y, -interp.z), color, linewidth);
        }
    }
    ...
    public static AxisAlignedBB getBoundingFromPos(final BlockPos pos) {
        ...
        final Vec3d interpolate = interpolateEntity(Minecraft.getMinecraft().player, Minecraft.getMinecraft().getRenderPartialTicks());
        return iBlockState.getSelectedBoundingBox(Minecraft.getMinecraft().world, pos).expand(0.0020000000949949026, 0.0020000000949949026, 0.0020000000949949026).offset(-interpolate.x, -interpolate.y, -interpolate.z);
    }
