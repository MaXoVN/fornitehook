    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (final EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            if (!player.isInvisible()) {
                ...
                output.put(healthSB + " " + (OyVey.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB + " \u00c2��f0", (int)EntityUtil.mc.player.getDistance(player));
    ...
    public static boolean getSurroundWeakness(final Vec3d pos, final int feetMine, final int render) {
        switch (feetMine) {
            ...
            case 5: {
                ...
                if (!BlockUtil.canBlockBeSeen(raytrace2.getX() - 1, raytrace2.getY(), raytrace2.getZ()) && Math.sqrt(EntityUtil.mc.player.getDistanceSq(raytrace2.getX() - 1, raytrace2.getY(), raytrace2.getZ())) > 3.0) {
                    return false;
                }
            ...
            case 7: {
                final BlockPos raytrace2 = new BlockPos(pos);
                if (!BlockUtil.canBlockBeSeen(raytrace2.getX(), raytrace2.getY(), raytrace2.getZ() - 1) && Math.sqrt(EntityUtil.mc.player.getDistanceSq(raytrace2.getX(), raytrace2.getY(), raytrace2.getZ() - 1)) > 3.0) {
                    return false;
                }
    ...
    public static EntityPlayer getTarget(final float range) {
        ...
        for (int size = EntityUtil.mc.world.playerEntities.size(), i = 0; i < size; ++i) {
            ...
            if (!isntValid(player, range)) {
                ...
                else if (EntityUtil.mc.player.getDistanceSq(player) < EntityUtil.mc.player.getDistanceSq(currentTarget)) {
                    currentTarget = player;
                }
            }
        }
