package me.fortnitehook.util.femhack;
...
public class SilentRotationUtil {
    ...
    public static void update(float f, float f2) {
        ...
        if (Util.mc.player == Util.mc.getRenderViewEntity()) {
            ...
            if (Util.mc.player.isRiding()) {
                Util.mc.player.connection.sendPacket(new CPacketPlayer.PositionRotation(Util.mc.player.motionX, -999.0, Util.mc.player.motionZ, f, f2, Util.mc.player.onGround));
                bl3 = false;
            } else if (bl3 && bl4) {
                Util.mc.player.connection.sendPacket(new CPacketPlayer.PositionRotation(Util.mc.player.posX, axisAlignedBB.minY, Util.mc.player.posZ, f, f2, Util.mc.player.onGround));
            } else if (bl3) {
                Util.mc.player.connection.sendPacket(new CPacketPlayer.Position(Util.mc.player.posX, axisAlignedBB.minY, Util.mc.player.posZ, Util.mc.player.onGround));
            } else if (bl4) {
                Util.mc.player.connection.sendPacket(new CPacketPlayer.Rotation(f, f2, Util.mc.player.onGround));
            } else if (((IEntityPlayerSP)Util.mc.player).getPrevOnGround() != Util.mc.player.onGround) {
                Util.mc.player.connection.sendPacket(new CPacketPlayer(Util.mc.player.onGround));
            }
            if (bl3) {
    ...
    public static void lookAtEntity(Entity entity) {
        float[] arrf = SilentRotationUtil.calcAngle(Util.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), entity.getPositionEyes(Util.mc.getRenderPartialTicks()));
        SilentRotationUtil.lookAtAngles(arrf[0], arrf[1]);
    }
    ...
    public static void lookAtXYZ(float f, float f2, float f3) {
        ...
        float[] arrf = SilentRotationUtil.calcAngle(Util.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), vec3d);
        SilentRotationUtil.setPlayerRotations(arrf[0], arrf[1]);
        Util.mc.player.rotationYawHead = arrf[0];
    }

    public static void lookAtBlock(BlockPos blockPos) {
        float[] arrf = SilentRotationUtil.calcAngle(Util.mc.player.getPositionEyes(Util.mc.getRenderPartialTicks()), new Vec3d(blockPos));
        SilentRotationUtil.setPlayerRotations(arrf[0], arrf[1]);
    ...
    public static void lookAtAngles(float f, float f2) {
        SilentRotationUtil.setPlayerRotations(f, f2);
        Util.mc.player.rotationYawHead = f;
    }
