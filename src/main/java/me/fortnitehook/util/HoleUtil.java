...
public class HoleUtil
    ...
    public static boolean is2x2(BlockPos blockPos) {
        return HoleUtil.is2x2(blockPos, true);
    }
    ...
    public static boolean isObbyHole(BlockPos blockPos) {
        ...
        for (BlockPos blockPos2 : holeOffsets) {
            ...
            if (!HoleUtil.isSafeBlock(blockPos.add(blockPos2))) {
                bl = false;
                continue;
            }
    ...
    public static Hole isDoubleHole(BlockPos blockPos) {
        if (HoleUtil.checkOffset(blockPos, 1, 0)) {
            return new Hole(false, true, blockPos, blockPos.add(1, 0, 0));
        }
        if (HoleUtil.checkOffset(blockPos, 0, 1)) {
    ...
    public static boolean isHole(BlockPos blockPos) {
        ...
        int n = 0;
        for (BlockPos blockPos2 : holeOffsets) {
            if (mc.world.getBlockState(blockPos.add(blockPos2)).getMaterial().isReplaceable())
                continue;
            ++n;
        }
        if (n == 5) {
    ...
    public static List<Hole> getHoles(double d, BlockPos blockPos, boolean bl) {
        ...
        List<BlockPos> list = HoleUtil.getSphere(d, blockPos, true, false);
        for (BlockPos blockPos2 : list) {
            Hole hole;
            if (mc.world.getBlockState(blockPos2).getBlock() != Blocks.AIR) continue;
            if (HoleUtil.isObbyHole(blockPos2)) {
                arrayList.add(new Hole(false, false, blockPos2));
                continue;
            }
            if (HoleUtil.isBedrockHoles(blockPos2)) {
                arrayList.add(new Hole(true, false, blockPos2));
                continue;
            }
            if (!bl || (hole = HoleUtil.isDoubleHole(blockPos2)) == null || mc.world.getBlockState(hole.pos1.add(0, 1, 0)).getBlock() != Blocks.AIR && mc.world.getBlockState(hole.pos2.add(0, 1, 0)).getBlock() != Blocks.AIR)
                continue;
            arrayList.add(hole);
        }
        return arrayList;
    ...
    public static boolean is2x2(BlockPos blockPos, boolean bl) {
        if (bl && !HoleUtil.isAir(blockPos)) {
            return false;
        }
        if (HoleUtil.is2x2Partial(blockPos)) {
            return true;
        }
        BlockPos blockPos2 = blockPos.add(-1, 0, 0);
        boolean bl2 = HoleUtil.isAir(blockPos2);
        if (bl2 && HoleUtil.is2x2Partial(blockPos2)) {
            return true;
        }
        BlockPos blockPos3 = blockPos.add(0, 0, -1);
        boolean bl3 = HoleUtil.isAir(blockPos3);
        if (bl3 && HoleUtil.is2x2Partial(blockPos3)) {
            return true;
        }
        return (bl2 || bl3) && HoleUtil.is2x2Partial(blockPos.add(-1, 0, -1));
    }

    public static boolean is2x1(BlockPos blockPos, boolean bl) {
        if (bl) {
            if (!HoleUtil.isAir(blockPos)) {
                return false;
            }
            if (!HoleUtil.isAir(blockPos.up())) {
                return false;
            }
            if (HoleUtil.isAir(blockPos.down())) {
                return false;
            }
        }
        int n = 0;
        for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
            ...
            if (HoleUtil.isAir(blockPos2)) {
                if (!HoleUtil.isAir(blockPos2.up())) {
                    return false;
                }
                if (HoleUtil.isAir(blockPos2.down())) {
                    return false;
                }
    ...
    public static boolean is2x1(BlockPos blockPos) {
        return HoleUtil.is2x1(blockPos, true);
    }

    public static boolean[] is1x1(BlockPos blockPos) {
        return HoleUtil.is1x1(blockPos, new boolean[]{false, true});
    }
    ...
    public static boolean is2x2Partial(BlockPos blockPos) {
        ...
        for (BlockPos blockPos2 : hashSet) {
            if (HoleUtil.isAir(blockPos2) && HoleUtil.isAir(blockPos2.up()) && !HoleUtil.isAir(blockPos2.down())) {
                if (HoleUtil.isAir(blockPos2.up(2))) {
                    bl = true;
                }
                for (EnumFacing enumFacing : EnumFacing.HORIZONTALS) {
    ...
    public static boolean checkOffset(BlockPos blockPos, int n, int n2) {
        return mc.world.getBlockState(blockPos).getBlock() == Blocks.AIR && mc.world.getBlockState(blockPos.add(n, 0, n2)).getBlock() == Blocks.AIR && HoleUtil.isSafeBlock(blockPos.add(0, -1, 0)) && HoleUtil.isSafeBlock(blockPos.add(n, -1, n2)) && HoleUtil.isSafeBlock(blockPos.add(n * 2, 0, n2 * 2)) && HoleUtil.isSafeBlock(blockPos.add(-n, 0, -n2)) && HoleUtil.isSafeBlock(blockPos.add(n2, 0, n)) && HoleUtil.isSafeBlock(blockPos.add(-n2, 0, -n)) && HoleUtil.isSafeBlock(blockPos.add(n, 0, n2).add(n2, 0, n)) && HoleUtil.isSafeBlock(blockPos.add(n, 0, n2).add(-n2, 0, -n));
    }

    public static boolean[] isHole(BlockPos blockPos, boolean bl) {
        boolean[] arrbl = new boolean[]{false, true};
        if (!HoleUtil.isAir(blockPos) || !HoleUtil.isAir(blockPos.up()) || bl && !HoleUtil.isAir(blockPos.up(2))) {
            return arrbl;
        }
        return HoleUtil.is1x1(blockPos, arrbl);
    }
