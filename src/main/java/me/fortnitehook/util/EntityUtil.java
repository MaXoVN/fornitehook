...
import me.fortnitehook.mixin.mixins.accessors.IEntityLivingBase;
...
public class EntityUtil
    ...
    public static void attackEntity(Entity entity, boolean packet) {
        /* 119 */     if (packet) {
            /* 120 */       mc.player.connection.sendPacket(new CPacketUseEntity(entity));
            /*     */     } else {
            /* 122 */       mc.playerController.attackEntity(mc.player, entity);
            /*     */     }
        /*     */   }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : mc.world.playerEntities) {
            ...
            output.put(healthSB + " " + (OyVey.friendManager.isFriend(player) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB + " \u00c2\u00a7f0", (int) mc.player.getDistance(player));
    ...
    public static EntityPlayer getTargetDouble(double range) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!(mc.player.getDistanceSq(player) < mc.player.getDistanceSq(currentTarget))) continue;
            currentTarget = player;
        }
