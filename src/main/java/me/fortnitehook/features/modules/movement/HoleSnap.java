...
public class HoleSnap
    ...
    public void onTick() {
        ...
        if (this.mode.getValue() == Mode.Instant) {
            blockPos2 = OyVey.holeManager.calcHoles().stream().min(Comparator.comparing(blockPos -> HoleSnap.mc.player.getDistance(blockPos.getX(), blockPos.getY(), blockPos.getZ()))).orElse(null);
            if (blockPos2 != null) {
        ...
        if (this.mode.getValue() == Mode.Motion) {
            ...
            double d = Math.toRadians(LuigiRotationUtil.getRotationTo(vec3d, vec3d2).x);
