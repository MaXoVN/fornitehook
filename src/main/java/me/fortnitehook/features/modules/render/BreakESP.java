...
import me.fortnitehook.event.events.BlockBreakEvent;
...
import me.fortnitehook.util.lover.FadeUtils;
...
public class BreakESP
    ...
    private final Setting<Integer> alphatwo;
    private final Setting<Integer> doubleAlphatwo;
    ...
    public void BlockBreak(BlockBreakEvent blockBreakEvent) {
        if (FortniteMine.godBlocks.contains(BreakESP.mc.world.getBlockState(blockBreakEvent.getPosition()).getBlock())) {
            return;
        }
    ...
    public void draw(BlockPos blockPos, double d, Color color, int n) {
        ...
        RenderUtil.drawBBFill(axisAlignedBB2, color);
    ...
    private void onRender3D5(double[] dArray, Render3DEvent render3DEvent, double[] dArray2, MinePosition minePosition) {
        ...
        if ((!minePosition.firstFinished || this.renderAir.getValue()) && (minePosition.miner != BreakESP.mc.player || this.renderSelf.getValue()) && minePosition.first.getDistance((int) BreakESP.mc.player.posX, (int) BreakESP.mc.player.posY, (int) BreakESP.mc.player.posZ) < this.range.getValue()) {
            ...
            this.draw(minePosition.first, dArray[0], new Color(this.red.getValue(), this.green.getValue(), this.blue.getValue(), this.alpha.getValue()), this.alphatwo.getValue());
            if (this.renderName.getValue() && (minePosition.miner != null || this.renderUnknown.getValue())) {
                ...
                this.renderNameTag(ChatFormatting.WHITE + (minePosition.miner == null ? "Unknown" : minePosition.miner.getName()), d3, d2, d, render3DEvent.getPartialTicks());
        ...
        if ((minePosition.miner != BreakESP.mc.player || this.renderSelf.getValue()) && !minePosition.secondFinished && minePosition.second != null) {
            if (BreakESP.mc.world.isAirBlock(minePosition.second)) {
                ...
                this.draw(minePosition.second, dArray2[0], new Color(this.doubleRed.getValue(), this.doubleGreen.getValue(), this.doubleBlue.getValue(), this.doubleAlpha.getValue()), this.doubleAlphatwo.getValue());
                if (this.renderName.getValue() && (minePosition.miner != null || this.renderUnknown.getValue())) {
                    ...
                    this.renderNameTag(ChatFormatting.WHITE + (minePosition.miner == null ? "Unknown" : minePosition.miner.getName()), d3, d2, d, render3DEvent.getPartialTicks());
                    this.renderNameTag(ChatFormatting.YELLOW + "Double", d3, d2 - 0.3, d, render3DEvent.getPartialTicks());
                }
            }
        }
    }
    ...
    public BreakESP() {
        super("BreakESP", "Show instant mine postion", Module.Category.RENDER, true, false, false);
        ...
        this.alphatwo = this.register(new Setting<>("AlphaOutline", 100, 20, 255));
        ...
        this.doubleAlphatwo = this.register(new Setting<>("DoubleAlpha2", 100, 20, 255, this::new4));
    }
