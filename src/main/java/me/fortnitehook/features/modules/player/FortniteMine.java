...
import me.fortnitehook.event.events.BlockEvent;
...
import me.fortnitehook.features.modules.combat.AntiBurrow;
...
import me.fortnitehook.util.lover.*;
...
public class FortniteMine extends Module {
    ...
    private static FortniteMine INSTANCE = new FortniteMine();
    private Block block;

    public FortniteMine() {
        super("FortniteMine", "breaking blocks", Category.PLAYER, true, false, false);
        ...
        this.bind = this.register(new Setting<>("ObsidianBind", new Bind(-1), (v) -> this.crystal.getValue()));
        ...
        this.renderMode = this.register(new Setting<>("FirstRenderMode", RenderMode.Outline));
    ...
    public static FortniteMine getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new FortniteMine();
        }
    ...
    public static void attackcrystal() {
        for(Object crystal : mc.world.loadedEntityList.stream().filter((e) -> e instanceof EntityEnderCrystal && !e.isDead).sorted(Comparator.comparing((e) -> mc.player.getDistance(e))).collect(Collectors.toList())) {
    ...
    public void onPacketSend(PacketEvent.Send event) {
        if (Feature.fullNullCheck()) {
            rend();
        } else if (breakPos != null && mc.player != null && mc.player.getDistanceSq(breakPos) > MathUtil.square(this.range.getValue().floatValue())) {
            breakPos = null;
            breakPos2 = null;
            cancelStart = false;
    ...
    public int getBestAvailableToolSlot(IBlockState blockState) {
        ...
        for(int i = 0; i < 9; ++i) {
            ...
            float speed;
            int eff;
            if (!stack.isEmpty && (speed = stack.getDestroySpeed(blockState)) > 1.0F && (double)(speed = (float)((double)speed + ((eff = EnchantmentHelper.getEnchantmentLevel(Enchantments.EFFICIENCY, stack)) > 0 ? Math.pow(eff, 2.0D) + 1.0D : 0.0D))) > max) {
                max = speed;
                toolSlot = i;
            }
        }
    ...
    public static void ondeve(BlockPos pos) {
        if (!Feature.fullNullCheck()) {
            if (me.fortnitehook.util.BlockUtil.canBreak(pos)) {
    ...
    public void onBlockEvent(BlockEvent event) {
        if (Feature.fullNullCheck()) {
            ...
            if (me.fortnitehook.util.BlockUtil.canBreak(event.pos)) {
    ...
    public void onRender3D(Render3DEvent event) {
        if (Feature.fullNullCheck()) {
            ...
            if ((breakPos != null || this.instant.getValue() && mc.world.getBlockState(breakPos).getBlock() == Blocks.AIR) && mc.player != null && mc.player.getDistanceSq(breakPos) > MathUtil.square(this.range.getValue().floatValue())) {
                breakPos = null;
                breakPos2 = null;
                cancelStart = false;
            } else if (me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE) != -1) {
                if (this.fuck.getValue() || me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE) != -1) {
                    if (doubleBreak.getValue() && breakPos2 != null) {
                        int slotMains = mc.player.inventory.currentItem;
                        if (mc.world.getBlockState(breakPos2).getBlock() != Blocks.AIR && me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE) != -1) {
                            if (mc.world.getBlockState(breakPos2).getBlock() == Blocks.OBSIDIAN && !breakSuccess.passedMs(1234L)) {
                                return;
                            }

                            mc.player.connection.sendPacket(new CPacketHeldItemChange(me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE)));
                    ...
                    if (!godBlocks.contains(mc.world.getBlockState(breakPos).getBlock())) {
                        if (this.ghostHand.getValue() && (this.fuck.getValue() || me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE) != -1) && me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE) != -1) {
                            ...
                            mc.player.inventory.currentItem = me.fortnitehook.util.InventoryUtil.getItemHotbar(Items.DIAMOND_PICKAXE);
                            mc.playerController.updateController();
                            mc.player.connection.sendPacket(new CPacketPlayerDigging(Action.STOP_DESTROY_BLOCK, breakPos, facing));
        ...
        if (!fullNullCheck()) {
            if (!mc.player.capabilities.isCreativeMode) {
                if (cancelStart) {
                    ...
                    if (this.bind.getValue().isDown() && this.crystal.getValue() && me.fortnitehook.util.InventoryUtil.findHotbarBlock(BlockObsidian.class) != -1 && mc.world.getBlockState(breakPos).getBlock() == Blocks.AIR) {
                        int obbySlot = me.fortnitehook.util.InventoryUtil.findHotbarBlock(BlockObsidian.class);
                        int old = mc.player.inventory.currentItem;
                        this.switchToSlot(obbySlot);
                        me.fortnitehook.util.BlockUtil.placeBlock(breakPos, EnumHand.MAIN_HAND, false, true, false);
                        this.switchToSlot(old);
                    }

                    if (breakPos != null && mc.player != null && mc.player.getDistanceSq(breakPos) > MathUtil.square(this.range.getValue().floatValue())) {
                        breakPos = null;
                        breakPos2 = null;
                        cancelStart = false;
                        ...
                        if (InventoryUtil.getItemHotbar(Items.END_CRYSTAL) != -1 && this.crystal.getValue() && mc.world.getBlockState(breakPos).getBlock() == Blocks.OBSIDIAN && !this.check() && !breakPos.equals(AntiBurrow.pos)) {
                        ...
                        if (this.ghostHand.getValue() || this.ghostHand.getValue() && this.fuck.getValue()) {
                            ...
                            int slotMain = mc.player.inventory.currentItem;
                            if (!breakSuccess.passedMs((int)breakTime)) {
                                return;
                            }
    ...
    public void onRenderWorld(RenderWorldLastEvent event) {
        if (fullNullCheck()) {
            ...
            if (this.render.getValue() && breakPos != null && cancelStart && this.render2.getValue()) {
                ...
                pos = pos.grow(0.002F).offset(-interpolateEntity.x, -interpolateEntity.y, -interpolateEntity.z);
